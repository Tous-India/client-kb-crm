...
const PurchaseOrderPrintPreview = forwardRef(({ purchaseOrder, companyDetails, bankDetails, globalRate = 83.5 }, ref) => {
  const exchangeRate = purchaseOrder?.exchange_rate || globalRate;
  ...
  const subtotal = purchaseOrder?.subtotal || items.reduce((sum, item) => sum + (item.total_price || item.requested_unit_price * item.quantity), 0);
  const tax = purchaseOrder?.tax || 0;
  const shipping = purchaseOrder?.shipping || 0;
  const grandTotal = purchaseOrder?.total_amount || (subtotal + tax + shipping);
  ...
  const customer = {
    name: purchaseOrder?.customer_name || purchaseOrder?.customer_id || 'Customer',
    customerId: purchaseOrder?.customer_id || '',
    notes: purchaseOrder?.customer_notes || ''
  };
    ...
    <div ref={ref} style={styles.container}>
      ...
      <div style={styles.header}>
        ...
        <div style={styles.titleSection}>
          <p style={styles.title}>PURCHASE ORDER</p>
          <p style={styles.poNumber}>{purchaseOrder?.po_number || 'PO-2024-001'}</p>
        </div>
      </div>
      ...
      <div style={styles.infoBar}>
        <div style={styles.infoBox}>
          <p style={styles.infoLabel}>PO Date</p>
          <p style={styles.infoValue}>{formatDate(purchaseOrder?.po_date)}</p>
        </div>
        <div style={styles.infoBox}>
          <p style={styles.infoLabel}>PO ID</p>
          <p style={styles.infoValueGray}>{purchaseOrder?.po_id || '-'}</p>
        </div>
        <div style={styles.infoBox}>
          <p style={styles.infoLabel}>Status</p>
          <p style={{ ...styles.infoValue, color: getStatusColor(purchaseOrder?.status), textTransform: 'uppercase', fontWeight: 700 }}>
            {purchaseOrder?.status || 'PENDING'}
          </p>
        </div>
        <div style={styles.infoBoxLast}>
          <p style={styles.infoLabelBlue}>Exchange Rate</p>
          <p style={styles.infoValueBlue}>$1 = ₹{exchangeRate.toFixed(2)}</p>
        </div>
      </div>
      ...
      <div style={styles.addressSection}>
        <div style={styles.addressBox}>
          <p style={styles.addressLabel}>From (Buyer)</p>
          <p style={styles.addressName}>{customer.name}</p>
          <p style={styles.addressText}>Customer ID: {customer.customerId}</p>
          {customer.notes && (
            <div style={styles.notesBox}>
              <p style={styles.notesLabel}>Customer Notes</p>
              <p style={styles.notesText}>"{customer.notes}"</p>
            </div>
          )}
        </div>
        <div style={styles.addressBox}>
          <p style={styles.addressLabel}>To (Supplier)</p>
      ...
      <div style={styles.table}>
        <div style={styles.tableHeader}>
          ...
          <span style={styles.colRate}>UNIT PRICE</span>
          <span style={styles.colUSD}>AMOUNT (USD)</span>
          <span style={styles.colINR}>AMOUNT (INR)</span>
        </div>
        {items.map((item, index) => {
          const unitPrice = item.requested_unit_price || item.unit_price || 0;
          const total = item.total_price || unitPrice * item.quantity;
          return (
            <div key={index} style={index % 2 === 0 ? styles.tableRow : styles.tableRowAlt}>
              ...
              <span style={{ ...styles.colRate, fontSize: '10px', color: '#7f8c8d' }}>${unitPrice.toFixed(2)}</span>
      ...
      <div style={styles.bottomSection}>
        <div style={styles.termsBox}>
          <p style={styles.sectionLabel}>Purchase Order Information</p>
          <p style={styles.termItem}>• This PO is subject to confirmation and stock availability.</p>
          <p style={styles.termItem}>• Prices are quoted in USD, Ex-Works basis.</p>
          <p style={styles.termItem}>• Delivery timeline will be confirmed upon order acceptance.</p>
          <p style={styles.termItem}>• Payment terms: As per quotation/agreement.</p>
          {purchaseOrder?.converted_to_quote_id && (
            <p style={styles.convertedText}>Converted to Quote: {purchaseOrder.converted_to_quote_id}</p>
          )}
        </div>
      ...
      <div style={styles.footer}>
        ...
        <p style={styles.footerSubtext}>This document was generated with exchange rate: $1 = ₹{exchangeRate.toFixed(2)}</p>
      </div>
    </div>
  );
});
